    ...
    <form id="form1" runat="server">
        ...
        <div class="rightinfo">
            ...
            <ul class="seachform">
                ...
                <li>
                    <label>日期</label><input name="" id="txtBeginTime" runat="server" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'txtEndTime\');}'})" type="text" class="scinput" />-
                        <input name="" id="txtEndTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'txtBeginTime\',{d:0});}',maxDate:'%y-%M-%d'})" runat="server" type="text" class="scinput" /></li>
            ...
            <table class="tablelist">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>直属会员</td>
                        <td><%=Convert.ToInt64(row["MemberWinScore"]) + Convert.ToInt64(row["MemberLostScore"])%></td>
                       <%-- <td><%=row["MemberTempTax"]%></td>
                        ...
                        <td><%=Convert.ToInt64(row["MemberWinScore"]) + Convert.ToInt64(row["MemberLostScore"])%></td>
                        <td><%=row["MemberProfit"]%></td>
                        <td><%=row["MemberProfit"]%></td>
                        <%--<td><%=row["MemberTaxProfit"]%></td>--%>
                        <%--<td><%=Convert.ToInt64(row["MemberProfit"]) + Convert.ToInt64(row["MemberTaxProfit"])%></td>--%>
                        <td><%=Convert.ToInt64(row["MemberWinScore"]) + Convert.ToInt64(row["MemberLostScore"]) + Convert.ToInt64(row["MemberTempTax"]) - Convert.ToInt64(row["MemberProfit"]) - Convert.ToInt64(row["MemberTaxProfit"])%></td>
                        <td><a class="tablelink" href="javascript:void(0)"   onclick ="getMemberProfitDetailClick()">查看盈亏明细</a></td>
                    </tr>
                    <tr>
                        <td>直属代理</td>
                        <td><%=Convert.ToInt64(row["NextProxyWinScore"]) + Convert.ToInt64(row["NextProxyLostScore"])%></td>
                   <%--     <td><%=row["NextProxyTempTax"]%></td>
                        <td><%=row2["StatScore"]%></td>--%>
                        <td><%=row2["RecNextProxy"]%></td>
                        <td><%=row["ProxyProfit"]%></td>
                        <td><%=Convert.ToInt64(row2["RecNextProxy"]) - Convert.ToInt64(row2["ParentTotalProfitWinLost2"])%></td>
                        <%--<td><%=row["ProxyTaxProfit"]%></td>--%>
                        <%--<td><%=Convert.ToInt64(row["ProxyProfit"]) + Convert.ToInt64(row["ProxyTaxProfit"])%></td>--%>
                        <td><%=row2["ParentTotalProfitWinLost2"]%></td>
                    ...
                    <tr style="background-color: #e5ebee">
                        ...
                        <td></td>
                        <td>统计</td>
                        <td><%=Convert.ToInt64(row["MemberWinScore"]) + Convert.ToInt64(row["MemberLostScore"]) + Convert.ToInt64(row["NextProxyWinScore"]) + Convert.ToInt64(row["NextProxyLostScore"])%></td>
                      <%--  <td><%=Convert.ToInt64(row["MemberTempTax"]) + Convert.ToInt64(row["NextProxyTempTax"])%></td>
                        <td><%=row["TotalWinLost"]%></td>--%>
                        <td><%=Convert.ToInt64(row2["RecNextProxy"]) + Convert.ToInt64(row["MemberWinScore"]) + Convert.ToInt64(row["MemberLostScore"])%></td>
                        <td><%=row["MyProfit"]%></td>
                        <%--<td><%=row["MyTaxProfit"]%></td>--%>
                        <%--<td><%=row["TotalProfitWinLost"]%></td>--%>
                        <td><%=Convert.ToInt64(row2["RecNextProxy"]) - Convert.ToInt64(row2["ParentTotalProfitWinLost2"])+Convert.ToInt64(row["MemberProfit"])%></td>
                        <td><%=Convert.ToInt64(row["TotalWinLost"]) - Convert.ToInt64(row["TotalProfitWinLost"])%></td>
                        <td></td>
                    </tr>
                    <%}
                        else
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("tbody td").each(function (k, v) {
            if (($(v).index() > 4 && $(v).index() != 8 || ($(v).parent().index() == 1 && $(v).index() > 0 && $(v).index() != 4)) && $(v).text() != "" && $(v).text() != "查看盈亏明细") {
                var text = $(v).text();
                if ($(v).text().indexOf("-") == 0) {
